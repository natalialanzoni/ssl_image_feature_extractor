...
def create_kaggle_dataset(
    ...
    # Collect only metadata (indices and labels) - NOT images!
    # This keeps memory usage low
    print("\nCollecting image metadata (not loading images yet)...")
    all_indices = list(range(len(hf_dataset)))
    all_labels = []
    all_class_names = []
    
    for idx in tqdm(all_indices, desc="Collecting metadata"):
        item = hf_dataset[idx]
        all_labels.append(item['label'])
        all_class_names.append(class_info[item['label']]['class_name'])
    
    print(f"Total images: {len(all_indices)}")
    
    # Shuffle indices (not images!)
    shuffled_indices = list(zip(all_indices, all_labels, all_class_names))
    ...
    # Save images (load, resize, save immediately - memory efficient!)
    def save_split(split_indices, split_name):
        ...
        for file_idx, (hf_idx, class_id, class_name) in enumerate(tqdm(split_indices, desc=f"{split_name}")):
            try:
                # Load image from HuggingFace dataset (only when needed!)
                item = hf_dataset[hf_idx]
                ...
                filename = f"{file_idx:06d}_class{class_id:03d}.jpg"
                ...
                print(f"\nWarning: Failed to process image {hf_idx}: {e}")
                continue
        
        return pd.DataFrame(metadata)
    
    # Save all splits (process one at a time to keep memory low)
